...
impl Tool for SearchLocalHistoryTool {
    fn name(&self) -> &str { "search_local_history" }
    fn description(&self) -> &str { "Search past Discord messages in this server. Use this when the user asks about past events or conversations that are not in the current context." }
    ...
    async fn execute(&self, params: Value) -> anyhow::Result<Value> {
        ...
        // Note: passing empty embedding since we are using keyword search fallback
        let results = self.db.search_messages(query, vec![], filter).await?;
