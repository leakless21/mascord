...
#[poise::command(slash_command, subcommands("list", "enable", "disable", "scope", "purge"))]
...
pub async fn list(ctx: Context<'_>) -> Result<(), Error> {
    ...
    if settings.is_empty() {
        ctx.say("📭 No custom channel settings found. All channels are using default (enabled).").await?;
        return Ok(());
    }
...
pub async fn enable(
    ctx: Context<'_>,
    #[description = "The channel to enable tracking for"]
    target_channel: serenity::Channel,
) -> Result<(), Error> {
    let guild_id = ctx.guild_id().ok_or("Must be run in a guild")?;
    ctx.data().db.set_channel_enabled(&guild_id.to_string(), &target_channel.id().to_string(), true)?;
    ctx.say(format!("✅ Memory tracking enabled for <#{}>.", target_channel.id())).await?;
    Ok(())
}
...
pub async fn disable(
    ctx: Context<'_>,
    #[description = "The channel to disable tracking for"]
    target_channel: serenity::Channel,
) -> Result<(), Error> {
    let guild_id = ctx.guild_id().ok_or("Must be run in a guild")?;
    ctx.data().db.set_channel_enabled(&guild_id.to_string(), &target_channel.id().to_string(), false)?;
    ctx.say(format!("❌ Memory tracking disabled for <#{}>. Messages in this channel will no longer be saved.", target_channel.id())).await?;
    Ok(())
}
...
pub async fn scope(
    ctx: Context<'_>,
    #[description = "The channel to configure scope for"]
    target_channel: serenity::Channel,
    #[description = "Start date (YYYY-MM-DD HH:MM:SS) or 'none' to reset"]
    date: String,
) -> Result<(), Error> {
    let guild_id = ctx.guild_id().ok_or("Must be run in a guild")?;
    let date_val = if date.to_lowercase() == "none" { None } else { Some(date) };
    
    ctx.data().db.set_channel_memory_scope(&guild_id.to_string(), &target_channel.id().to_string(), date_val.clone())?;
    
    match date_val {
        Some(d) => ctx.say(format!("✅ Memory scope for <#{}> set to start from **{}**.", target_channel.id(), d)).await?,
        None => ctx.say(format!("✅ Memory scope for <#{}> reset to full history.", target_channel.id())).await?,
    };
    Ok(())
}
...
pub async fn purge(
    ...
    let reply = ctx.send(poise::CreateReply::default()
        ...
        .components(vec![serenity::CreateActionRow::Buttons(vec![
            serenity::CreateButton::new("confirm").label("Confirm Purge").style(serenity::ButtonStyle::Danger),
            serenity::CreateButton::new("cancel").label("Cancel").style(serenity::ButtonStyle::Secondary),
        ])])
    ).await?;

    if let Some(interaction) = reply.message().await?.await_component_interaction(ctx.serenity_context()).author_id(ctx.author().id).await {
        if interaction.data.custom_id == "confirm" {
            let count = ctx.data().db.purge_messages(&channel.id().to_string(), before_date)?;
            interaction.create_response(ctx.serenity_context(), serenity::CreateInteractionResponse::UpdateMessage(
                serenity::CreateInteractionResponseMessage::new()
                    .content(format!("🗑️ Purged **{}** messages from <#{}>.", count, channel.id()))
                    .components(vec![])
            )).await?;
        } else {
            interaction.create_response(ctx.serenity_context(), serenity::CreateInteractionResponse::UpdateMessage(
...
pub async fn get(ctx: Context<'_>) -> Result<(), Error> {
    let guild_id = ctx.guild_id().ok_or("Must be run in a guild")?;
    info!("Settings get command: Fetching context settings for guild {}", guild_id);
...
pub async fn set(
    ...
    info!("Settings set command for guild {}: updated {}", guild_id, confirmations.join(", "));
...
pub async fn summarize(ctx: Context<'_>) -> Result<(), Error> {
    info!("Manual summarization triggered for channel {} by user {}", ctx.channel_id(), ctx.author().name);
    ctx.defer().await?;
    ...
    info!("Manual summarization for channel {} completed successfully", ctx.channel_id());
